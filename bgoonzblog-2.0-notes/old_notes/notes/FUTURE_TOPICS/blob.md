...
Methods that perform web-requests, such as [XMLHttpRequest](xmlhttprequest.html), [fetch](fetch.html) and so on, can work with `Blob` natively, as well as with other binary types.
